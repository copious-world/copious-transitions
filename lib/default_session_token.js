...
class LocalSessionTokens {
   ...
   from_token(token) {
    ...
    async add(session_token,ownership_key) {
        // src_key e.g. augmented hash token  OR hh unidentified (an intermediate hash of the value == ownership_key )
    ...
    async current_active(token,is_sess_req) {
        ...
        if ( (key !== undefined) ) {
            ...
            try {
                if ( is_sess_req ) {
                    key = await this.db.get_session_key_value(token)  // session token
                } else {
...
module.exports.token_maker = default_token_maker
