...
class EndpointManager extends ForeignAuthExtension {
    ...
    initialize_endpoint_services(conf) {
        ...
        this.endpoint_service.set_transition_handler((msg_obj) => {
            ...
            if ( server_id && this.trans_processor ) {      // transitional
                ...
                let result = this.trans_processor.endpoint_transition(transition,msg_obj)
...
class GeneralTransitionEngImpl extends WebSocketActions {
    ...
    async upload_chunk(post_body,files) {
        ...
        if (  f_keys.length === 1 ) {         // Handling one file
            ...
            // until post bodies indicate that there is no more data to come.s
