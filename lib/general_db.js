...
class GeneralDBWrapperImpl extends AppLifeCycle {
    ...
    async store_user(udata,key,cb) { // up to the application...(?)
        ...
        let result = await this.pdb.update(udata,dont_remote,'create')   // the end point will write information into files and create directories...
    ...
    async update_user(udata,key,cb) {     // just work with the presistent DB
        ...
        let dont_remote = false
        let result = await this.pdb.update(udata,dont_remote,'update')   // the end point will write information into files and create directories...
        if ( cb !== undefined ) cb(result)
    }
