...
class GeneralDBWrapperImpl extends AppLifeCycle {
    ...
    async initialize(conf) {
        ...
        await this.key_value_db.initialize(conf.db.key_value_db)
        if ( this.session_key_value_db !== this.key_value_db ) {
            await this.session_key_value_db.initialize(conf.db.session_key_value_db)
        }
        await this.pdb.initialize(conf.db.persistence_db)           // initialize persitence...relative to the database
        await this.sdb.initialize(conf.db.static_db)
        //
    ...
    /**
     * Use admin tools to establish endpoints for admin messages
     * 
     * Application can override
     * 
     * @param {object|Array} endpoints 
     * @param {object} cnf - may be used by an override
     */
    async establish_admin_endpoints(endpoints,cnf) {
        if ( Array.isArray(endpoints) ) {
            this.admin_endpoint = {}
            for ( let ep of endpoints ) {
                this.admin_endpoint[ep.key] = ep
            }
        } else {
            this.admin_endpoint = endpoints            
        }
    }
    ...
    async change_key_value_db_instance(new_kv_db,conf) {
        if ( this.key_value_db ) {
            ...
            await this.remove_key_value_db_connection(conf)  // conf is passed for those types that keep a number of connections opend
        }
        this.key_value_db = new_kv_db
        if ( new_kv_db && !(new_kv_db.initialized) ) {
    ...
    async add_key_value_db_connection(conf) {
        if ( this.key_value_db && (typeof this.key_value_db.add_connection === 'function') ) {
    ...
    async remove_key_value_db_connection(conf) {
        if ( this.key_value_db && (typeof this.key_value_db.close_connection === 'function') ) {
    ...
    async change_session_key_value_db_instance(new_session_db,conf) {
        if ( this.key_value_db ) {
            conf._db_closing = true
            await this.remove_session_key_value_db_connection(conf)  // conf is passed for those types that keep a number of connections opend
        }
        this.session_key_value_db = new_session_db
        if ( new_session_db && !(new_session_db.initialized) ) {
    ...
    async add_session_key_value_db_connection(conf) {
        if ( this.session_key_value_db && (typeof this.session_key_value_db.add_connection === 'function') ) {
    ...
    async remove_session_key_value_db_connection(conf) {
        if ( this.session_key_value_db && (typeof this.session_key_value_db.close_connection === 'function') ) {
            conf._db_closing = true
    ...
    async change_static_db_instance(new_static_db,conf) {
        if ( this.sdb ) {
            conf._db_closing = true
            await this.remove_static_db_connection(conf)  // conf is passed for those types that keep a number of connections opend
        }
        this.sdb = new_static_db
        if ( new_static_db && !(new_static_db.initialized) ) {
    ...
    async add_static_db_connection(conf) {
        if ( this.sdb && (typeof this.sdb.add_connection === 'function') ) {
    ...
    async remove_static_db_connection(conf) {
        if ( this.sdb && (typeof this.sdb.close_connection === 'function') ) {
            conf._db_closing = true
    ...
    async change_persistence_db_instance(new_persistence_db,conf) {
        if ( this.pdb ) {
            conf._db_closing = true
            await this.remove_persistence_db_connection(conf)  // conf is passed for those types that keep a number of connections opend
        }
        this.pdb = new_persistence_db
        if ( new_persistence_db && !(new_persistence_db.initialized) ) {
    ...
    async add_persistence_db_connection(conf) {
        if ( this.pdb && (typeof this.pdb.add_connection === 'function') ) {
    ...
    async remove_persistence_db_connection(conf) {
        if ( this.pdb && (typeof this.pdb.close_connection === 'function') ) {
            conf._db_closing = true
    ...
    async set_session_key_value(key,value) {   // notice this is a hash set (a speedy check)
        return await this.session_key_value_db.hash_set(key,value)  // example: session_token, ccwid -> link hash
    }
    ...
    async check_hash(hh_invertable, value) {    // example: link_hash, ccwid
