...
class GeneralDBWrapperImpl extends AppLifeCycle {
    ...
    constructor(keyValueDB,sessionKeyValueDB,persistentDB,staticDB) {
        ...
        if ( !(persistentDB) ) {    // DISK within LOCAL and PEER NODES
            this.pdb = require('../default_storage/default_persistent_db')
        } else {
            this.pdb = persistentDB
        }
        //
        if ( !(staticDB) ) {        // LOCAL ON DISK (LOCAL CACHE AND STARTUP CACHE)
            this.sdb = require('../default_storage/default_static_db')
        } else {
            this.sdb = staticDB
        }
    ...
    initialize(conf) {
        ...
        this.key_value_db.initialize(conf.key_value_db)
        if ( this.session_key_value_db !== this.key_value_db ) {
            this.session_key_value_db.initialize(conf.session_key_value_db)
        }
        this.pdb.initialize(conf.persistence_db)           // initialize persitence...relative to the database
        this.sdb.initialize(conf.static_db)
