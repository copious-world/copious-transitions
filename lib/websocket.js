...
class WebSocketManager {
    ...
    initialize(conf,app) {
        this.app = app
        this.conf = conf
        this.class_conf = conf.web_sockets

        let cconf = this.class_conf

        if ( cconf.web_socket_class ) {
            const WebSocket = require(cconf.web_socket_class)
            const WebSocketServer = WebSocket.Server;
            this.WebSocket =  WebSocket
            this.WebSocketServer = WebSocketServer
        } else if (cconf.web_socket_server_class ) {
        ...
        //
        if ( cconf.http_module ) {
            this.http = require(cconf.http_module);
        } else {
        ...
        //
        if ( conf && cconf.ports && cconf.ports.port_names ) {
            let port_names = cconf.ports.port_names
            if ( Array.isArray(port_names) && (typeof cconf.ports.defs === 'object') ) {
                let defs = cconf.ports.defs
                for ( let pname of port_names ) {
                    if ( (defs[pname] !== undefined) && defs[pname] ) {
    ...
    add_service(port) {
        ...
        server.listen(port);
        return new this.WebSocketServer({ "server" : server });
    }

    add_service_instance(w_server,pname) {
        if ( typeof this.port_handlers[pname] === 'function' ) {
            this.port_handlers[pname](w_server)
        }
    }


    change_http_service_creator(new_http) {
        this.http = new_http
    }

    change_websocket_server_class(new_websocket_class) {
        this.WebSocketServer = new_websocket_class
    }
