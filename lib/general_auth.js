...
class SessionManager extends SessionManager_Lite {
    ...
    addSession(key,session_token) {
        if ( (key !== undefined) && (session_token !== undefined) ) {
            let src_key = this.db.set_session_key_value(session_token,key)
            this.goingSessions[src_key] = session_token
            this.goingTokens[session_token] = key
        }
    }
    ...
    async registration_transition(post_body,transtion_object) {
        if ( post_body.password ) {
            ...
            await this.db.store_user(post_body)         /// CREATE USER ENTRY IN DB
