    ...
    */
    ...
    stash_session_token(user,transtion_object,sess_tok) {  // sess_tok a made up token (app rule)
        ...
        if ( app_sess_data_access ) {
            this.release_session_data[sess_tok] = user[app_sess_data_access]  // app overrides so that it knows this field (ucwid)
        ...
        switch ( user_op ) {
            ...
            case 'logout' : {
                if ( this.db ) {
                    ...
                    transtion_object.secondary_action = false
                    transtion_object.session_token = 'sess+gone'   // going to whip out the session idenitifer in the rest of execution
