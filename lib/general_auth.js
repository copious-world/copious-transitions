...
class SessionManager extends AppLifeCycle {
            ...
            case 'logout' : {
                if ( this.db ) {
                    ...
                    if ( token && this.tokenCurrent(token) ) {
                        ...
                        //
                        //res.cookie(this.user_cookie,'', { maxAge: 0, httpOnly: true }); // delete the cookie
                    }
                }
                break;
            }
    ...
    async update_user_password(post_body) {
        if ( this.db ) {
            try {
                if ( post_body.token ) {
                    let keyed_val = this.db.get_key_value(post_body.token)  // information that links the user to the reset password
                    if ( keyed_val ) {
                        let query = {}
                        ...
                        if ( user ) {
                            if ( user.password && post_body.password ) {
                                ...
                                if ( await this.password_check(user.password,post_pass) ) {
                                    ...
                                    if ( updates ) {
                                        user.password = update.pass
                                        delete user.updates.pass
                                        if ( Object.keys(use.updates).length == 0 ) delete user.updates
                                        this.db.update_user(user)
                                        return(keyed_val)
                                    }
                                }
