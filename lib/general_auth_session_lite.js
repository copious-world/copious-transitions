...
class SessionManager_Lite extends SessionTokenManager {
    ...
    constructor(exp_app,db_obj,business,transition_engine,tokenStorage) {  // reference to the DB and initializes the a middleware vector
        ...
        let class_conf = conf ? conf.authorizer : false

        super(class_conf,db_obj,tokenStorage)
        //
        this.class_conf = class_conf
        this.conf = conf
        //
        ...
        //
        this.user_cookie = class_conf.user_cookie
        this.max_age_user_cookie = 90000
        this.require_secure_transfer = false
        //
        if ( class_conf ) {
            if ( class_conf.use_secure_transfer ) {
     ...
     * @returns string
     */
    selected_session_table_hash(token,ucwid) {  // applications must return the hash used by their storage table
        let hash = do_hash(token+ucwid)
        return hash
    }
     ...
     * @returns boolean
     */
    async session_guard(token,ucwid) {
        let active = await this.sessionCurrent(token,ucwid)
        if ( !active ) {  // give it a second chance...
            let link_hash = this.selected_session_table_hash(token,ucwid)
            this.insert_session(session_token,link_hash)
            active = await this.sessionCurrent(token,ucwid)
            if ( !active ) {
     ...
     */
    process_asset(asset_id,post_body,dynamic) {
        let token = this.generate_transition_token(asset_id)
        let asset_type = dynamic ? "dynamic_asset" : "static_asset" 
        let transition_object = this.create_transition_record(asset_type)
    ...
    constructor(sessClass,tokenStorageClass) {
        ...
        if ( tokenStorageClass !== undefined ) {
            if ( (typeof tokenStorageClass.token_maker) === 'function' ) {
                this._l_token_maker = token_maker
            }
        }
    }

    sessions(exp_app,db_obj,business,transition_engine) {
        let sess_m = new this.sessionClass(exp_app,db_obj,business,transition_engine,this.tokenStorageClass);
        this.db = db_obj
        return(sess_m)
    }
}
