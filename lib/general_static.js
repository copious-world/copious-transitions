...
class GeneralStatic extends AppLifeCycle {
        ...
        if ( asset_key ) {
            ...
            if ( asset ) return(asset)
                ...
                if ( asset ) return(asset)
                    ...
                    return(this.db.static_store(asset_key))
                }        
            }
        }
    ...
    generic_prep_cwd_offset(conf) {
        ...
        if ( application_specific_loc[application_specific_loc.length - 1] === '/' ) {
            application_specific_loc = application_specific_loc.substr(0,application_specific_loc.length-1)
        }
        if ( application_specific_loc[0] !== '/' ) {
            application_specific_loc = '/' + application_specific_loc
        }
        let root_dir = process.cwd()
        if ( application_specific_loc ) {
            for ( let asset in this.preloaded ) {
                let assetDescr = this.preloaded[asset]
